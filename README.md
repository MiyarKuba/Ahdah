...
This repository is currently in **Settlement DDL artifacts prepared — awaiting review and separate execution approval**. The [DDL review](docs/SETTLEMENT_DDL_REVIEW.md) and [artifact set](database/settlement/README.md) provide 18 SQL files for 26 proposed tables, five existing-table changes, verification, staged cutover, rollback and rehearsal. Policy v1 remains authoritative; the physical design was approved for DDL preparation only. The approved physical-design correction separates source-evidenced unassigned Opening from Manager-approved project Designate; Policy v1 is unchanged. No schema or application change has been applied. The delivered Flutter settlement route remains read-only, and financial closure remains disabled.

`GET /api/v1/projects/{projectId}/settlement` returns blocker references, category totals, evaluation gaps and distinct settlement/closure impediments. `canSettle` and `canClose` are nullable: false means blocked; null means indeterminate. Zero known blockers does not establish readiness because approved attribution and finalization policies are not yet implemented in the read-side contract. See [docs/SETTLEMENT.md](docs/SETTLEMENT.md) for the authoritative contract and limitations.
