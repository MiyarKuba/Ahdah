...
## Flutter settlement boundary

`features/settlements` follows the existing domain/data/presentation split: handwritten immutable HTTP models, `SettlementRepository`/`ApiSettlementRepository`, and one project-scoped auto-disposed Riverpod controller. The adapter delegates authenticated GET and exact financial JSON decoding to `ApiClient`; it sends no company, actor, role, financial total, or state authority. Models preserve `bool?` and decimal text, never derive readiness or combine totals, and reject malformed financial payloads.

Project Details links to the named `projectSettlement` route (`/projects/:projectId/settlement`). Dedicated `canViewProjectSettlement` capability guards both entry and route; server tenant/assignment policies remain authoritative. The page renders separate settlement/closure states and impediments, visible blocker counts, prominent evaluation gaps, and expandable evaluated categories. Hidden and unattributable categories suppress nested financial data. Supervisor presentation additionally restricts company-financial categories. Stable codes have English/Arabic mappings and an uncertainty fallback.

The controller prevents duplicate reads, discards results after disposal, and is recreated when the session changes. Refresh retains the snapshot only while the new request is pending; failure removes stale financial data. A 401 uses centralized session expiry; 403/404/network failures remain localized retryable states. Blocker navigation maps record type plus safe ID to existing named routes and permissions; API `resourcePath` never becomes a client route. There are no settlement commands, caches, background retries, or persistence-model dependencies.
