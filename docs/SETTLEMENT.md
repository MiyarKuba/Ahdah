...
The documentation-only [settlement policy decision review](SETTLEMENT_POLICY_DECISIONS.md) now evaluates advance attribution, machine-testable certification predicates, snapshots, permissions, post-close restrictions, cancellation, reopening and concurrency. Its D01–D16 policy baseline is now approved with the owner’s D01/D06/D08/D11 clarifications. Approved policy includes controlled custody release, configurable paper requirements, qualifying undisputed contractual owner retention outside immediate cash settlement, and independent approval without a permanent two-Manager requirement. Policy approval does not change this endpoint’s current false/null semantics or authorize schema execution.
