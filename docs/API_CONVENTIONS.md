...
Suppliers Phase 1 uses the same mechanism for payment-account creation, supplier invoice/debt creation, supplier payment creation/confirmation/rejection, credit-note creation/approval, and credit allocation. Supplier directory creation and metadata PATCH are not financial commands. Collections use page 1/default 20/maximum 100. Account identifiers are masked; no generated entity, tenant ID, authoritative outstanding/funding balance input, payer identity, raw audit JSON, or ledger ID crosses the public contract.
