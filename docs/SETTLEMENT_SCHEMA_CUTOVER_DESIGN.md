...
**DDL preparation follow-up (2026-09-17):** the owner approved this physical design for artifact preparation only. [Settlement DDL review](SETTLEMENT_DDL_REVIEW.md) and [database/settlement](../database/settlement/README.md) now contain all 26 table definitions, five existing-table alterations, guarded verification/cutover/rollback artifacts and rehearsal specifications. No DDL has been executed. Existing-table enforcement is staged after reviewed legacy population. The owner-approved physical-design correction below resolves T03/T12 Opening scope: source-evidenced unassigned Opening, then separate Manager-approved Designate. Policy v1 is unchanged.
...
| T03 `custody_operations` | operation_type; status; requested_by_user_id; requested_at; approved_by_user_id?; approved_at?; authorization_decision_id?; reason; idempotency_record_id; source_operation_id?; money_transfer_id?; expense_id?; expense_return_id?; legacy_attribution_review_id?; advance_settlement_resolution_id?; version_number | DB: unique(company_id,idempotency_record_id,operation_type); type Opening, Designate, Release, Reallocate, ReserveExpense, ConsumeExpense, RejectExpense, ReserveTransfer, ConfirmTransfer, RejectTransfer, Restore, Correction; status Draft, PendingApproval, Posted, Rejected, Cancelled. Posted header immutable; correction references prior operation. FKs typed to roots; type-specific required references via CHECK. APP validates full participant set, permission and posting; Manager project authorization mandatory for Designate/Release/Reallocate and project-affecting Correction (CorrectCustody). Opening requires null project authorization and confirmed delivery or approved evidenced legacy Balance review; only unassigned effects. Company-only Correction uses approved existing balance-resolution or legacy-review authority. |
...
| T12 `project_authorization_decisions` | project_id; action_code; actor_user_id; actor_role; decided_at; decision; policy_version_id; preparer_user_id?; submitter_user_id?; beneficiary_user_id?; conflict_detected boolean; conflict_reason?; evidence_id?; delegation_reference? uuid; request_hash; details jsonb object | Immutable. DB action Prepare, Submit, Approve, Close, Reopen, Acknowledge, Complete, Designate, Release, Reallocate, CorrectCustody, Exception, ReconcileOwner; decision Allowed/Denied; role Manager/Deputy/Accountant/Supervisor; FK actors/evidence/policy. Delegation reference must be null under v1; no unsupported FK to a nonexistent delegation model. APP active membership, visibility, beneficiary conflict and action matrix. BOTH stored identity inequality plus current authorization checks. |
...
| W29 | Advance delivery confirm: ConfirmTransferAsync→ConfirmDeliveryAsync / AdvancesController | unassigned Opening; later separate Designate | transfer, advance, funding sources; V; creates balance | Y*/Y*/Y*/possible | L; zero destination + unassigned opening; no inference |
...
| W36 | Designate/release/reallocate/project Correction: F | explicit source/destination projects | F | Y/Y/Y/yes | L; Accountant/Deputy prepares, Manager approves |
...
4. Obtain holder declarations of **current available and reserved** custody by project/unassigned, with evidence. Accountant/Deputy reviews; independent Manager approves separate Designate operations. Do not infer remaining custody from historical expense ratios, supplier identity or source provenance. Historical postings remain untouched.
5. T25 records each source/boundary/version/digest; T26 captures affected projects. Approved Opening T03/T04 entries establish unassigned current custody, not a replay of historical gross receipts. Separate Manager-approved Designate operations establish reviewed project slices; preserve historical attribution evidence in T25/T26 and the Designate source. Persist baseline history reconciliation separately in evidence. For an uncertain balance use a provisional unassigned slice to conserve arithmetic, mark review Indeterminate and explicitly state that unassigned does not certify ownership.
6. Pending transfers/returns keep amounts reserved at sender until actual confirmation; no destination opening receipt and no double custody. Link original allocation to final T05 slices only when reviewed, in the same cutover transaction as unassigned Opening and separate approved Designate, then force deferred conservation. Never reparent an existing live reservation. Unresolved reservation attribution stays a gap for affected projects and prevents their closure. Pending delivery with no existing custody stays a source reservation, not an invented balance. Reconcile pending expenses similarly.
...
**Exact next task:** review the prepared [DDL artifacts](SETTLEMENT_DDL_REVIEW.md), including the owner-approved physical-design correction. Only after separate staging-execution authorization, perform isolated installation/catalog verification and structural rehearsal. No production mutation, EF scaffold or command implementation is authorized by this design milestone.
